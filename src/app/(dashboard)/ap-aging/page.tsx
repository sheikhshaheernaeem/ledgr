  ...
  return (
    <div className="p-8 space-y-6 print:p-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        <Card className="border-red-500/20 bg-red-500/5 col-span-2 md:col-span-3 lg:col-span-1">
        ...
        {BUCKETS.map((b) => (
          <Card key={b} className={bucketCardStyles[b]}>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground mt-0.5">
                {rows.filter((r) => r.bucket === b).length} bill
                {rows.filter((r) => r.bucket === b).length !== 1 ? "s" : ""}
              </p>
            </CardContent>
          </Card>
        ))}
        ...
        <Card className="border-border bg-card">
          <CardContent className="py-16 text-center text-muted-foreground">
            ...
            <p className="text-sm mt-1">
              All bills are paid or none are pending yet.
            </p>
          </CardContent>
        </Card>
      ) : (
          ...
          <CardContent className="pt-4">
            <div className="flex flex-wrap gap-4 text-sm">
              <div className="font-semibold text-foreground">
                Total Payable:{" "}
                <span className="text-red-400">{fmt(totalOutstanding)}</span>
              </div>
              {BUCKETS.map((b) =>
                bucketTotals[b] > 0 ? (
